...
The project explicitly recognizes the limitations of AI-generated source scenes: occlusion, missing scale, and model capability can reduce fidelity at the appearance, dimension, and equipment-model levels. The proposed workflow therefore replaces AI placeholders with components from a user-owned model library while preserving instance identity, real-asset bindings, and runtime behavior. See [model-library and placeholder replacement design](docs/MODEL_LIBRARY.md).
